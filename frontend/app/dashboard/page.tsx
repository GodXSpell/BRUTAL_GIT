    ...
    <div className="bg-background text-on-background min-h-screen flex flex-col font-body-md">
      ...
      <header className="bg-zinc-950 docked full-width top-0 border-b-2 border-white shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] flex items-center justify-between px-6 py-3 w-full z-50 sticky">
        <div className="flex items-center gap-6">
          <Link
            href="/"
            className="text-2xl font-black uppercase text-white font-['Space_Grotesk'] font-bold tracking-tighter hover:text-[#238636] transition-colors"
          >
            BRUTAL_GIT
          </Link>
          <div className="relative hidden md:flex items-center w-64 border-2 border-white bg-[#010409] focus-within:border-primary-container focus-within:border-[3px]">
            <span className="material-symbols-outlined absolute left-2 text-zinc-400 text-sm">
              search
            </span>
            <input
              className="w-full bg-transparent border-none text-white pl-8 pr-2 py-1 focus:ring-0 text-sm font-label-sm placeholder-zinc-500"
              placeholder="Search..."
              type="text"
            />
            <div className="absolute right-2 px-1 border border-zinc-600 text-zinc-500 text-[10px] rounded flex items-center justify-center">
              /
            </div>
          </div>
        </div>
        <nav className="hidden md:flex items-center gap-4">
          <a
            className="text-zinc-400 hover:bg-zinc-800 hover:text-white transition-none px-2 py-1 font-label-sm uppercase"
            href="#"
          >
            Pull requests
          </a>
          ...
          <a
            className="text-[#238636] border-b-2 border-[#238636] hover:bg-zinc-800 hover:text-white transition-none px-2 py-1 font-label-sm uppercase"
            href="#"
          >
            Explore
          </a>
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="text-zinc-400 hover:text-white border-2 border-zinc-400 hover:border-white w-6 h-6 flex items-center justify-center">
      ...
      <main className="flex-grow w-full max-w-7xl mx-auto px-gutter py-12 grid grid-cols-1 md:grid-cols-12 gap-gutter items-start">
        ...
        <div className="md:col-span-9 flex flex-col gap-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-2 pb-4 border-b-[3px] border-white mt-6">
            <h2 className="font-headline-lg text-[24px] text-white uppercase m-0">
              Recommended for You
            </h2>
          ...
          {error && (
            <div className="border-2 border-[#ffb4ab] bg-[#93000a] text-[#ffdad6] p-4 brutal-shadow-level-2 uppercase font-bold text-sm">
              <span className="material-symbols-outlined mr-2 inline-block align-middle">
                error
              </span>
              {error}
            </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col border-2 border-white bg-surface brutal-shadow-level-3 p-12 items-center justify-center gap-4 text-zinc-400">
              ...
              <span className="font-mono text-white text-sm uppercase">
                No recommendations found. Adjust filters.
              </span>
            </div>
          ) : (
                ...
                {recommendations.map((rec, index) => (
                  ...
                  >
                    <RepoCard
                      recommendation={rec}
                      onFeedback={(signal) =>
                        recordFeedback(rec.repo.fullName, signal, index)
                      }
                    />
                  </motion.div>
                ))}
